...
class AvvoApi::BaseTest < Test::Unit::TestCase
  ...
  context "A resource that inherits from AvvoApi::Base" do
    ...
    context "with a belongs_to association and correct parameters" do
      ...
      should "hit the Avvo API with the correct URL when saved" do
        stub_request(:post, "https://api.avvo.com/api/1/lawyers/2/specialties.json")
        @object.save
        assert_requested(:post, "https://api.avvo.com/api/1/lawyers/2/specialties.json")
      end
      
      should "hit the Avvo API with the correct URL when retrieved" do 
        stub_request(:get, "https://api.avvo.com/api/1/lawyers/2/specialties/3.json").to_return(:body => {:id => '3'}.to_json)
        AvvoApi::Specialty.find(3, :params => {:lawyer_id => 2})
        assert_requested(:get, "https://api.avvo.com/api/1/lawyers/2/specialties/3.json")      
      end

      should "hit the Avvo API with the correct URL when updated" do 
        stub_request(:get, "https://api.avvo.com/api/1/lawyers/2/specialties/3.json").to_return(:body => {:id => '3', :specialty_id => "88", :specialty_id => '88', :specialty_percent => '100'}.to_json)
        license = AvvoApi::Specialty.find(3, :params => {:lawyer_id => 2})
        assert_requested(:get, "https://api.avvo.com/api/1/lawyers/2/specialties/3.json")  
        stub_request(:put, "https://api.avvo.com/api/1/lawyers/2/specialties/3.json")
        license.save
        assert_requested(:put, "https://api.avvo.com/api/1/lawyers/2/specialties/3.json")
      end

      should "set the prefix parameters correctly when saved" do
        stub_request(:post, "https://api.avvo.com/api/1/lawyers/2/specialties.json").to_return(:body => {:id => '2', :specialty_id => "88", :specialty_id => '88', :specialty_percent => '100'}.to_json)
        @object.save
        assert_requested(:post, "https://api.avvo.com/api/1/lawyers/2/specialties.json")
